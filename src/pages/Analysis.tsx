...
const Analysis = () => {
  ...
  const validateFile = (file: File) => {
    const allowedTypes = ['application/pdf', 'text/csv', 'image/jpeg', 'image/jpg', 'image/png'];
  ...
  const updateAnalysisSession = async (sessionId: string, updates: any) => {
    const {
      error
    } = await supabase.from('analysis_sessions').update(updates).eq('session_id', sessionId);
    if (error) {
  ...
  const BulkFileUploadField = () => <div className="space-y-4">
      <div className="border-2 border-dashed border-primary/30 rounded-lg p-8 text-center hover:border-primary/50 transition-colors bg-primary/5">
        <input id="bulk-upload" type="file" accept=".pdf,.csv,.jpg,.jpeg,.png" onChange={e => handleMultipleFileSelection(e.target.files)} className="hidden" disabled={isAnalyzing} multiple />
        <label htmlFor="bulk-upload" className="cursor-pointer flex flex-col items-center space-y-4">
      ...
      {files.length > 0 && <div className="space-y-3">
          ...
          <div className="grid gap-2">
            {files.map((file, index) => <div key={index} className="flex items-center justify-between p-3 bg-card border border-border rounded-lg">
                ...
                <Button variant="ghost" size="sm" onClick={() => removeFile(index)} className="h-8 w-8 p-0 text-muted-foreground hover:text-red-500" disabled={isAnalyzing}>
                  <X className="w-4 h-4" />
                </Button>
              </div>)}
          </div>
  ...
  const FileUploadField = ({
    ...
    index: number;
      ...
      <div className="border-2 border-dashed border-border rounded-lg p-6 text-center hover:border-primary/50 transition-colors relative">
        <input id={`file-${index}`} type="file" accept=".pdf,.csv,.jpg,.jpeg,.png" onChange={e => handleFileChange(index, e.target.files?.[0] || null)} className="hidden" disabled={isAnalyzing} />
        ...
        {files[index] && <div className="mt-2 flex items-center justify-center space-x-2 text-sm text-green-600">
            <CheckCircle className="w-4 h-4" />
            <span className="flex-1 truncate">{files[index].name}</span>
            <Button variant="ghost" size="sm" onClick={e => {
          e.preventDefault();
          removeFile(index);
        }} className="h-6 w-6 p-0 text-red-500 hover:text-red-700" disabled={isAnalyzing}>
              <X className="w-4 h-4" />
            </Button>
          </div>}
      </div>
  ...
  return <div className="min-h-screen bg-gradient-to-br from-fintech-navy via-fintech-navy to-fintech-navy">
      <Navigation />
      ...
      <main className="pt-20 pb-20">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="cosmic-card rounded-2xl p-8 shadow-2xl">
            ...
            {/* Progress Steps */}
            {isAnalyzing && <div className="mb-8">
                ...
                return <div key={key} className="flex flex-col items-center space-y-2">
                        <div className={`w-10 h-10 rounded-full flex items-center justify-center ${status === 'completed' ? 'bg-green-500 text-white' : status === 'active' ? 'bg-blue-500 text-white' : 'bg-gray-200 text-gray-500'}`}>
                          <Icon className={`w-5 h-5 ${status === 'active' ? 'animate-spin' : ''}`} />
                        </div>
                        <span className={`text-xs ${status === 'completed' ? 'text-green-600' : status === 'active' ? 'text-blue-600' : 'text-gray-500'}`}>
                          {label}
                        </span>
                      </div>;
              })}
            ...
            <div className="space-y-8">
              ...
              {files.length === 0 ? (/* Bulk Upload */
            <BulkFileUploadField />) : (/* Show selected files and option to add more */
            <div className="space-y-6">
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center space-x-2">
                        ...
                        <Input id="count" type="number" min="1" max="10" value={statementCount} onChange={e => setStatementCount(parseInt(e.target.value) || 1)} className="h-8 w-20" disabled={isAnalyzing} />
                      </div>
                    </div>
                    ...
                    <div className="space-y-4">
                      {Array.from({
                    length: Math.max(statementCount, files.length)
                  }, (_, index) => <FileUploadField key={index} index={index} />)}
                    </div>
                  </div>
                </div>)}

              {/* Upload Progress */}
              {uploadProgress.length > 0 && <div className="space-y-2">
                  <h3 className="font-medium">Upload Progress:</h3>
                  {uploadProgress.map(progress => <div key={progress.fileName} className="flex items-center space-x-2 text-sm">
              ...
              {/* ZIP Creation Status */}
              {isCreatingZip && <div className="flex items-center space-x-2 text-sm text-blue-600">
                  <Loader2 className="w-4 h-4 animate-spin" />
                  <span>Creating ZIP archive...</span>
                </div>}

              {zipError && <div className="flex items-center space-x-2 text-sm text-red-600">
                  <AlertCircle className="w-4 h-4" />
                  <span>ZIP creation failed: {zipError}</span>
                </div>}

              {/* Analyze Button */}
              <Button onClick={handleAnalyze} disabled={isAnalyzing || isUploading || files.length === 0} className="w-full bg-primary hover:bg-primary/90 text-primary-foreground h-12">
                {isAnalyzing ? <>
